...
object RemoteActors extends App {
  val localSystem = ActorSystem(
    "LocalSystem",
    ConfigFactory.load("part2_remoting/remoteActors.conf")
  )
  ...
  remoteActorRefFuture.onComplete {
    case Success(actoreRef) => actoreRef ! "I've resolved you in a future"
   ...
   * - actor resolver will send Identify(42)
   * - the remote actor will AUTOMATICALLY response with ActorIdentity(42, actorRef)
